...
const Alert5 = ({ cusInfo, customer, alertShow }) => {
    ...
    const deleteCustomer = async (customerInfo) => {
        try {
            const doctorRef = doc(db, "customers", customerIds[customerInfo - 1]);
    ...
    return (
        <>
            {!alert2Show ? <div className="Alert5">
                <div className="title">
                    <h3>
                        {
                            !cusInfo.delate ? "Bekor qillingan mijoz" :
                                cusInfo.complated ? "Kutayotgan mijoz" :
                                    !cusInfo.complated ? "Tugatilgan mijoz" : ""
                        }
                    </h3>
                ...
                </div>
                {
                    !cusInfo.delate ?
                        <button
                            onClick={() => reyloadCustomer2(cusInfo.id)}
                            className="reyload"
                        >
                        ...
                        cusInfo.complated ?
                            <div className="weitButtons">
                                <button
                                    onClick={() => deleteCustomer(cusInfo.id)}
                                    style={{ background: "#ff1216" }}
                                >
                                    Bekor qilish <RiDeleteBin5Fill />
                                </button>
                                <button
                                    onClick={() => setalert2Show(true)}
                                    style={{ background: "#5f53fe" }}
                                >
                                    O'zgartirish <CiEdit />
                                </button>
                                <button
                                    onClick={() => complatedCustomer(cusInfo.id)}
                            ...
                            !cusInfo.complated ?
                                <button
                                    onClick={() => reyloadCustomer1(cusInfo.id)}
                                    className="reyload"
                                >
